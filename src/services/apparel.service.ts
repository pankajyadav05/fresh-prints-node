...
class ApparelService {
  ...
  async bulkUpdateApparelBySize(stocks: Stock[]) {
    for (let stock of stocks) {
      ...
      if (!foundApparel)
        throw new AppError(
          APPAREL_NOT_FOUND,
          `No Apparel found with code ${stock.code}`,
          404
        );
  ...
  async checkIfStockExists({ stockQuality, size, code }) {
    ...
    if (!foundApparel)
      throw new AppError(
        APPAREL_NOT_FOUND,
        `No Apparel found with code ${code}`,
        404
      );
    ...
    if (sizeIndex === -1) return { isStock: false };
